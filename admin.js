...
const loadParcel = async ()=>{
   try {
        const {data} = await axios.get('https://parcel-delivery.herokuapp.com/api/v1/parcels', {
         headers: {
        ...
        const parcel = userParcel.map((parcel)=>{
          const{destination, status, presentLocation, _id:parcelId, createdBy:userId} = parcel
          return` 
            <article class="item">
